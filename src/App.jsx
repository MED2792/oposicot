...
export default function OposicotTest() {
  ...
  const handleAuthSubmit = async (e) => {
    ...
    if (authMode === "signup") {
      ...
      if (error) {
        ...
        setAuthNotice("Cuenta creada. Revisa tu email para confirmarla antes de iniciar sesión.");
      }
    } else {
  ...
  // Guarda los resultados en Supabase en cuanto se llega al resumen
  useEffect(() => {
    if (stage !== "summary" || !session || answers.length === 0) return;
    setSavingResults(true);
    const rows = BLOCKS.filter((b) => stats.byBlock[b.id].total > 0).map(
      (b) => ({
        user_id: session.user.id,
        block: b.id,
        correct: stats.byBlock[b.id].correct,
        total: stats.byBlock[b.id].total,
      })
    );
    supabase
      .from("quiz_results")
      .insert(rows)
      .then(({ error }) => {
        if (error) console.error("Error guardando resultados:", error.message);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: "14px", flexWrap: "wrap" }}>
          ...
          {session && (
            <span
              className="osc-mono"
              style={{ fontSize: "12px", color: "#5C7A88", display: "flex", alignItems: "center", gap: "8px" }}
            >
              {session.user.email}
              <button
      ...
      <div style={{ padding: "28px", maxWidth: "720px", margin: "0 auto" }}>
        {authLoading && (
          <p style={{ color: "#5C7A88", fontSize: "14px" }}>Cargando…</p>
        )}
        ...
        {!authLoading && !session && (
          <div style={{ maxWidth: "360px", margin: "40px auto" }}>
            <div
              className="osc-display"
              style={{ fontSize: "17px", fontWeight: 600, marginBottom: "6px", textAlign: "center" }}
            >
              {authMode === "login" ? "Inicia sesión" : "Crea tu cuenta"}
            </div>
            <p style={{ color: "#8FA0A8", fontSize: "13px", textAlign: "center", marginBottom: "20px" }}>
              Necesitas una cuenta para guardar tu progreso entre sesiones
            </p>
            <form onSubmit={handleAuthSubmit} style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              ...
              />
              {authError && (
                <p style={{ color: "#B56B5C", fontSize: "13px", margin: 0 }}>{authError}</p>
              )}
              {authNotice && (
                <p style={{ color: "#7FA88F", fontSize: "13px", margin: 0 }}>{authNotice}</p>
              )}
              <button
                type="submit"
                className="osc-btn"
